...
**2026-07-14追記（Issue #22、2〜3時間の長時間入力実測で決着）**:
| 同一kotoba v2.0系重み | 5分 | 10分 | 2時間 |
|---|---|---|---|
| mlx-whisper | 15s/404字 | 27s/**136字**（5分単体より減る＝内容崩壊） | **4時間18分経過で未完了**＋スワップ枯渇 |
| faster-whisper(CPU int8) | 223s/806字 | 319s/1,643字（線形で健全） | 推定60-90分（実時間の1.3〜1.9倍しか出ない） |
| **whisper.cpp(Metal q5_0)** | **21s/746字** | **39s/1,247字（ほぼ線形）** | **推定9分** |
- 結論: **Macの本線はwhisper.cpp**（fasterの品質×mlxの速度）。autoの解決を cpp→faster に変更し、mlxは非推奨・明示指定のみに格下げ（12.1のランタイム序列訂正「llama.cpp系が遅い」はVLMの話であり、ASRエンコーダ・デコーダ小型モデルでは当てはまらない点に注意）
